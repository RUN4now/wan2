...
export const ProductModal = memo(function ProductModal({ product, isOpen, onClose, allProducts }: ProductModalProps) {
  ...
  const currentIndex = useMemo(() =>
    allProducts.findIndex(p => p.id === currentProduct.id),
  ...
  const handleBuyNow = useCallback(() => {
    ...
    onClose();
    openCart();
  }, [addToCart, currentProduct, onClose, openCart]);

  const handlePrevProduct = useCallback((e: React.MouseEvent) => {
    e.stopPropagation();
    if (currentIndex > 0) {
  ...
  const handleNextProduct = useCallback((e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const cleanDescription = useCallback((desc: string): string => {
    return desc
      .replace(/\*\*/g, '')
      .replace(/\n{3,}/g, '\n\n')
      .trim();
  ...
  return (
    <div
      ...
      style={{ background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(8px)', willChange: 'opacity' }}
      onClick={onClose}
    >
      <div className="relative flex items-center justify-center w-full h-full">
        {currentIndex > 0 && (
          <button
            ...
            style={{ background: 'var(--surface)', willChange: 'transform, opacity' }}
          >
        ...
        <div
          ...
          style={{ background: 'var(--surface)', color: 'var(--text)', willChange: 'transform' }}
